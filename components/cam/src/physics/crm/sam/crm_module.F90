...
! Liran_________________
    integer         :: uvcount
    real(crm_rknd)  :: umean, vmean
! Liran_________________
...
! Liran Here____________________________
...
!  vmean = 0.0
!  do k = 1 , nzm
!    if ((pres(icrm,k).gt.700.0).and.(pres(icrm,k).lt.900)) then
!      do j = 1 , ny
!        do i = 1 , nx     
!          umean = umean + crm_state_u_wind     (icrm,i,j,k) 
!          vmean = umean + crm_state_v_wind     (icrm,i,j,k)
!          uvcount  = uvcount + 1
!        enddo
!      enddo
!    endif
...
!      do j = 1 , ny
!        do i = 1 , nx
!          crm_state_u_wind(icrm,i,j,k) = umean/uvcount
!          crm_state_v_wind(icrm,i,j,k) = umean/uvcount   
!        enddo
!      enddo
!    endif
!  enddo
...
!--------------------------------------------------
  ...
  nstop = dt_gl/dt
  dt = dt_gl/nstop
!write(iulog,*) "Liran Check Here",dt_gl,dt,nstop
  crm_run_time  = dt_gl
  ...
  do while (nstep < nstop)
    ...
    do icrm = 1 , ncrms
      do k=1,nzm
        ...
        !write(iulog,*) 'Liran check
        !ww=>',ncrms,icrm,l,crm_ww_inst(icrm,l),wbaraux(icrm,l)
      enddo
    enddo
